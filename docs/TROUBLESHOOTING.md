...
* Соединения в логе есть, а подключения нет — посмотрите на провод:
  ...
  | есть | 0 | смотрите SYN-ACK: нет ответов — порт закрыт в firewall; ответы есть — приветствие теряется |
  | есть | есть | провод в порядке, приветствия отвергает сам Xray |

### Снаружи сервер мёртв, а изнутри работает

Признак: `vpnctl selftest` проходит, а с устройств не подключается; в
`vpnctl capture` попытки соединения есть, ответов сервера (SYN-ACK) нет.

Почти всегда это расхождение между портом, который слушает сервис, и портом,
разрешённым в nftables. Разъезжаются они, например, если порт меняли через
`./install.sh --only xray --vless-port ...` — сервис переехал, правила остались.

```bash
vpnctl firewall status    # сверяет слушающие порты с правилами
vpnctl firewall sync      # перегенерирует правила под текущие порты
```

Почему `selftest` этого не ловил раньше: он подключается к собственному
публичному адресу сервера, а такой трафик возвращается через `lo` и правила
портов не проходит. Теперь порт проверяется отдельным шагом.
