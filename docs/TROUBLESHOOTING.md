...
### Мелкое ходит, крупное — нет (битый PMTU)

Признак: TCP-соединение устанавливается (SYN-ACK есть), но TLS-приветствие не
доходит или приходит обрезанным; при этом Hysteria и AmneziaWG работают —
у них пакеты меньше.

Размер пакетов клиента задаётся значением MSS, которое сервер объявляет в
SYN-ACK. Если по пути теряются крупные пакеты, приветствие (у современных
браузеров и клиентов оно около 1700–2000 байт из-за постквантового ключа)
не доходит целиком.

```bash
vpnctl set-mss 1360     # заставить клиента резать приветствие помельче
vpnctl set-mss off      # вернуть как было
```

Проверять ступенчато: 1360, при неудаче 1200, затем 1000. Если помогает —
дело было в MTU, а не в блокировках.
