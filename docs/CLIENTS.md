...
## Как переносить конфиги на устройство

**Сканируйте QR-код, а не копируйте текст.** Ссылки длинные и содержат `&`, `#`
и base64 — мессенджеры и терминалы их портят: обрезают по `#`, добавляют
разметку, переносят строки. Приложение при этом часто не ругается, а молча
импортирует то, что разобрало, и соединение не работает.

```bash
vpnctl show <имя> --qr
```

Если всё же копируете текстом — проверьте, что вставилось целиком, до самого
`#имя` в конце.
...
После импорта **обязательно откройте профиль в приложении и убедитесь, что
поля REALITY заполнены** — `security` должен быть `reality`, а `publicKey` и
`shortId` непустыми. Если они пусты, ссылка импортировалась не полностью:
в логе сервера это выглядит как `REALITY: failed to read client hello`
(проверяется командой `vpnctl debug log`). Если полей REALITY в приложении
вообще нет — оно слишком старое, возьмите другое из таблицы выше.
...
Отдельная запись на устройство обязательна **только для AmneziaWG**: там ключ
клиента жёстко связан с его адресом в туннеле, и два устройства с одним ключом
будут перебивать друг друга.

VLESS и Hysteria 2 к этому равнодушны — один UUID или один логин/пароль
спокойно обслуживает несколько устройств одновременно. Так что чисто
технически для них можно держать одну общую учётку.

Цена такого решения одна: **нельзя отозвать доступ у одного устройства.**
Потеряли телефон — меняете учётку всем сразу. Если это не пугает, заводите
общий профиль для Hysteria, а отдельные записи — только под AmneziaWG:

```bash
vpnctl add common                 # общий: все три протокола
vpnctl add phone  --awg --qr      # только AmneziaWG
vpnctl add laptop --awg --qr
vpnctl add ipad   --awg --qr
```

Ключ `--only` принимает список: `--only awg,vless` заведёт всё, кроме Hysteria.

Если ни у одного клиента не останется учётки Hysteria, её сервис
останавливается — с пустым списком пользователей он всё равно не стартует.
Поднимется сам, как только такой клиент появится.
